...
##
def leave(node):
	mesh_id = mesh_ids_by_node[node]
	mesh = meshes_by_id[mesh_id]
	del mesh_ids_by_node[node]
	mesh.remove(node)
	if len(mesh) == 0:
...
def parse_message(node, message):
	...
	send_to_mesh(meshes_by_id[mesh_ids_by_node[node]], node, forward_message)
